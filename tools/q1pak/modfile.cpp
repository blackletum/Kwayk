...
void ModFile::loadTextures(QDataStream &stream, const Lump &lump)
{
    ...
    for (int i = 0; i < nummiptex; ++i) {
        ...
        if (name.startsWith("sky")) {
            texture.name = name;
            texture.srcWidth = mt.width;
            texture.srcHeight = mt.height;
...
void ModFile::skyLoadTexture(const Texture &texture)
{
    ...
    texture1.srcWidth = 128;
    texture1.srcHeight = 128;
    ...
    texture2.srcWidth = 128;
    texture2.srcHeight = 128;
...
void ModFile::buildSurfaceDisplayList(const Surface &surface, Subset &subset)
{
    ...
    if (!(surface.flags & DrawFlag::Tiled)) {
        ...
        // BSP UVs are in original texel space; image may be scaled to POT for WebGL.
        sdiv = float(surface.texinfo->texture->srcWidth);
        tdiv = float(surface.texinfo->texture->srcHeight);
    }
...
QImage ModFile::createTexture(const QByteArray &pixels, int width, int height, TexPref flags) const
{
    ...
    const int scaledWidth = nextPowerOfTwo(width);
    const int scaledHeight = nextPowerOfTwo(height);

...
import "../js/avertexnormals.js" as Normals

ParticleSystem3D {
    id: root

    property var avelocities: []

    Component.onCompleted: {
        const av = [];
        for (let i = 0; i < Normals.avertexnormals.length; ++i)
            av.push([Math.random() * 2.56, Math.random() * 2.56, Math.random() * 2.56]);
        avelocities = av;
    }
    ...
    ParticleEmitter3D {
        id: brightfieldParticleEmitter
        particle: brightfieldParticle
        lifeSpan: 150
        velocity: VectorDirection3D {
            direction: Qt.vector3d(0, 0, 0)
        }
    ...
    function entityParticles(pos) {
        const norms = Normals.avertexnormals;
        for (let i = 0; i < norms.length; ++i) {
            let angle = main.time * avelocities[i][0];
            let sp = Math.sin(angle);
            let cp = Math.cos(angle);
            angle = main.time * avelocities[i][1];
            const sy = Math.sin(angle);
            const cy = Math.cos(angle);
            const n = norms[i];
            const org = Qt.vector3d(pos.x + n[0] * 0.64 + cp * cy * 0.16,
                                    pos.y + n[2] * 0.64 + sp * -0.16,
                                    pos.z - n[1] * 0.64 - cp * sy * 0.16);
            brightfieldParticleEmitter.particleScale = calcParticleScale(org) * 2;
            brightfieldParticle.color = d_8to24table[0x6f];
            adjustParticleGamma(brightfieldParticle, 0.5);
            brightfieldParticleEmitter.burst(1, 0, org);
        }
    }
}

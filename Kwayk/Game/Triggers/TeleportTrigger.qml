...
Trigger {
    ...
    function spawn_tfog(org) {
        const tfog = tfogComponent.createObject(world, { position: org });
        tfog.remove.connect(() => { tfog?.destroy() })
        particles.teleportSplash(org);
    }

    function spawn_tdeath(org, death_owner) {
        const tdeath = tdeathComponent.createObject(world, { position: org, owner: death_owner });
        tdeath.remove.connect(() => { tdeath?.destroy() })
    }

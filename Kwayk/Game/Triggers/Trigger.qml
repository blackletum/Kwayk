    ...
    th.onPhysics: {
        if (touch !== null && solid) {
            ...
            if (world && world.player) {
                ...
                let pmins = world.player.mins;
                let pmaxs = world.player.maxs;

                let tmin = root.position.plus(root.mins);
                let tmax = root.position.plus(root.maxs);

                if (p.x + pmaxs.x >= tmin.x && p.x + pmins.x <= tmax.x &&
                    p.y + pmaxs.y >= tmin.y && p.y + pmins.y <= tmax.y &&
                    p.z + pmaxs.z >= tmin.z && p.z + pmins.z <= tmax.z) {
